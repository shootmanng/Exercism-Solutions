...
sub set_player_scores (%new_scores) {
    while (my ($key, $value) = each %new_scores) {
        # Issue: %Scores{$key} = $value; does not work because
        # it tries to assign the whole hash and causes an error,
        # need to assign the SCALAR associated with that key.
        $Scores{$key} = $value;
    }
    return;
}
